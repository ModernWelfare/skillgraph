...
public class CPT
{
	...
	//should be consistent with the array, with the exception of manually setting the size
	private int size;
	...
	public CPT(int numberOfParents, CPTSkillTemplate cptTemplate)
	{
		...
		for (int i=0; i<size; i++)
		{
			...
			double lowerBound = cptTemplate.getLowerBound(i, numberOfOnes);
			double upperBound = cptTemplate.getUpperBound(i, numberOfOnes);
	...
	//potentially dangeous, but might be useful later if we want to manually do things
	...
	//return the probability that the [(skill is known) || (student guessed) || (student slipped) || (item is known)]
	...
	public void setProbability(String binaryString, int trueEvent, double probability)
	{
		if(probability < 0 || probability > 1)
		{
			System.err.println("Probability range must be between 0 and 1");
	...
	public boolean equals(Object otherCPT)
	{
		...
		else
		{
			...
			if(sizeCheck(probabilityValues, otherProbabilities))
			{
				...
				{
					for(int j=0; j<probabilityValues[i].length && returnValue; j++)
					{
