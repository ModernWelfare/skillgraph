...
public class SkillGraph {
	...
	public void mergeSkills(int parentSkillIndex, int childSkillIndex) {
		...
		// add the child's children to the parent's children list
		for (Skill s : child.getChildren()) {
			if (!parent.getChildren().contains(s)) {
				parent.getChildren().add(s);
			}
			// remove child as the parent of its children
			s.getParents().remove(child);
		...
		// add the child's parents to the parent's parent List
		for (Skill s : child.getParents()) {
			if (!s.equals(parent)) {
				if (!parent.getParents().contains(s)) {
					parent.getParents().add(s);
				}

				s.getChildren().remove(child);

				if (!s.getChildren().contains(parent)) {
					s.getChildren().add(parent);
				}
			}
		}
