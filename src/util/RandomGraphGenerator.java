...
public class RandomGraphGenerator {
	...
	/**
	 * for each number of nodes rank the nodes an edge cannot be drawn from
	 * higher rank to lower rank that eliminates isomorphisms
	 * 
	 * iteration0 = start with unconnected graph with rankes nodes [save this
	 * graph, I=0] iteration1 (edge number) = generate all possible graphs with
	 * one edge [save these graphs, I=2] iteration2 (edge number) = for each
	 * saves graph of the previous iteration - generate all possible graphs with
	 * one more edge [save these graphs] repeat until no more edges can be drawn
	 * 
	 * no cycle check needed connected check level check
	 * 
	...
	public static int[][] makeRandomMatrix(int skillNumLowerBound,
		...
		// this initialization of array lists of generic primitives compiles
		...
		for (int i = skillNumLowerBound; i <= skillNumUpperBound; i++) {
			// holds all skill matrices generated for a specific number of
			// skills
			...
			// holds skill matrices for the previous and current number of edges
			// in the graph
			...
			while (!previousEdgeMatrices.isEmpty()) {
				// for all skill graphs with a number of edges, add one more
				// edge in all possible ways
					...
					for (int k = 0; k < numberOfSkills - 1; k++) {
						for (int l = k + 1; l < numberOfSkills; l++) {
							if (currentSkillMatrix[k][l] == 0) {
								int[][] newSkillMatrix = GraphFunctions
										.matrixCopy(currentSkillMatrix);
			...
			// System.out.println("Number ofiteration  graphs = " +
			// iterationSkillMatrices.size());
		}
		...
		for (int j = 0; j < allSkillMatrices.size(); j++) {
			...
			if (GraphFunctions.isMatrixConnected(currentSkillMatrix)
					&& GraphFunctions.isLevelInRange(currentSkillMatrix,
		...
		int randomGraphIndex = ConstantRNG.getNextInt(0,
				allValidSkillMatrices.size() - 1);
	...
	private static void outputGuessAndSlipFile(int numberOfItems,
		...
		for (int i = 0; i < numberOfItems; i++) {
			String minGuess = Double.toString(Constants.minGuess);
			String maxGuess = Double.toString(Constants.maxGuess);
			String minSlip = Double.toString(Constants.minSlip);
			String maxSlip = Double.toString(Constants.maxSlip);
